...
void maxheapify(HeapType *h, int i, int heapsize) {
	...
	if (left >= heapsize)
		...
		if (left< (heapsize) && h->heap[left].key>h->heap[i].key) {
			largest = left;
		}
		else
			largest = i;
		if (right<(heapsize) && h->heap[right].key > h->heap[largest].key)
			largest = right;
		if (largest != i) {
			temp = h->heap[i].key;
			h->heap[i].key = h->heap[largest].key;
			h->heap[largest].key = temp;
			maxheapify(h, largest, heapsize);
		}
	}
}
