...
void huffman_encoding(char *str, bits_stream *bits_str)
{
	...
	for (int i = 0; i < len; i++) {
		...
		case 'a':
			j = 1;
			for (int m = 0; m < m_bit_size[0]; m++) {
				bits_str->stream[k] = m_LUT[0][m];
				k++;
			}
			break;
		...
		case 'b':
			j = 3; 
			for (int m = 0; m < m_bit_size[1]; m++) {
				bits_str->stream[k] = m_LUT[1][m];
				k++;
			}
			break;
		case 'c':
			j = 3;
			for (int m = 0; m < m_bit_size[2]; m++) {
				bits_str->stream[k] = m_LUT[2][m];
				k++;
			}
			break;
		case 'd':
			j = 3;
			for (int m = 0; m < m_bit_size[3]; m++) {
				bits_str->stream[k] = m_LUT[3][m];
				k++;
			}
			break;
		...
		case 'e':
			j = 4;
			for (int m = 0; m < m_bit_size[4]; m++) {
				bits_str->stream[k] = m_LUT[4][m];
				k++;
			}
			break;
		case 'f':
			j = 4;
			for (int m = 0; m < m_bit_size[5]; m++) {
				bits_str->stream[k] = m_LUT[5][m];
				k++;
			}
			break;
		}
